const apiKey = "openai"; // Clave API para OpenAI

// Variables de la API de vuelo
const flightApiKey = "apikey";
const flightApiSecret = "apisecret";
...
function sendMessage() {
  ...
  if (message) {
    ...
    document.getElementById(
      "chatbox"
    ).innerHTML += `<div><strong>Yo:</strong> ${message}</div>`;
    input.value = "";
    ...
    if (message.toLowerCase().includes("vuelo")) {
      ...
      const flightDetails = message.match(
        /de (\w+) a (\w+) el (\d{2})\/(\d{2})\/(\d{4})/
      ); // Expresión regular para "día/mes/año"
      ...
      if (!flightDetails) {
        ...
        document.getElementById(
          "chatbox"
        ).innerHTML += `<div><strong>Bot:</strong> ¡Genial! Estoy aquí para ayudarte con los vuelos. Por favor, proporciona los detalles en el formato 'de ORIGEN a DESTINO el DÍA/MES/AÑO' y la cantidad de personas (1,2,3..) que tomaran el vuelo.</div>`;
        document.getElementById("chatbox").scrollTop =
          document.getElementById("chatbox").scrollHeight;
        return;
      }
      ...
      searchFlights(
        ...
        .then((flights) => {
          ...
          document.getElementById(
            "chatbox"
          ).innerHTML += `<div><strong>Bot:</strong> ${flightsText}</div>`;
          document.getElementById("chatbox").scrollTop =
            document.getElementById("chatbox").scrollHeight;
        })
        .catch((error) => {
          ...
          document.getElementById(
            "chatbox"
          ).innerHTML += `<div><strong>Bot:</strong> Hubo un error al buscar vuelos.</div>`;
          document.getElementById("chatbox").scrollTop =
            document.getElementById("chatbox").scrollHeight;
      ...
      fetch("https://api.openai.com/v1/chat/completions", {
        ...
        .then((data) => {
          ...
          document.getElementById(
            "chatbox"
          ).innerHTML += `<div><strong>Bot:</strong> ${reply}</div>`;
          document.getElementById("chatbox").scrollTop =
            document.getElementById("chatbox").scrollHeight;
          isTyping = false; // Resetear el estado de "escribiendo..." después de la respuesta
        })
        .catch((error) => {
          ...
          document.getElementById(
            "chatbox"
          ).innerHTML += `<div><strong>Error:</strong> No se pudo obtener respuesta del bot.</div>`;
        });
    }
  }
}
